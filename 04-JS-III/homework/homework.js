...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  if (arguments.length === 0){
    ...
    acummulator = acummulator * arguments[i];
    }
  return acummulator;
  }
}
...
function diaDeLaSemana(numeroDeDia) {
  ...
  var infoDelDia = ""
    if (numeroDeDia === 1 || numeroDeDia === 7) {
      infoDelDia = "Es fin de semana";
    } else {
      infoDelDia = "Es dia Laboral";
    }
  return infoDelDia;
} 
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí
  var itemBefore = arreglo[0];
  var lastItem = arreglo.length - 1;
  for (i = 1; i < arreglo.length; i++) {
    if (arreglo[i] === arreglo [i - 1]) {
      itemBefore = arreglo[i]
    }
  }
  if (arreglo[lastItem] === itemBefore) {
    return true;
  } else {
    return false;
  }
